...
import {
  Building2, Cpu, Thermometer, Activity, Plus, MapPin,
  ChevronRight, X, Trash2, AlertTriangle, DollarSign, Gauge, TrendingDown,
} from 'lucide-react'
import {
...
import type { Zone, Bill } from '../lib/api'
...
export default function FleetOverview() {
  ...
  const [portfolio, setPortfolio] = useState<PortfolioData>({
    equipmentCounts: {}, zones: [], bills: [], loaded: false,
  })
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      Promise.all(facilities.map(f => api.listZones(f.id).then(r => r.zones).catch(() => []))),
      Promise.all(facilities.map(f => api.listBills(f.id).then(r => r.bills).catch(() => []))),
    ]).then(([eqResults, zoneResults, billResults]) => {
      const counts: Record<string, number> = {}
      eqResults.forEach(({ id, count }) => { counts[id] = count })
      setPortfolio({
        equipmentCounts: counts,
        zones: zoneResults.flat(),
        bills: billResults.flat(),
        loaded: true,
      })
    })
  }, [facilities])

  const totalEquipment = Object.values(portfolio.equipmentCounts).reduce((a, b) => a + b, 0)
  ...
  portfolio.zones.forEach(z => {
    ...
    zonesByState[key] = (zonesByState[key] || 0) + 1
  })
  ...
  return (
    <div className="page-container">
      ...
      <div className="stat-grid stagger">
        ...
        <StatCard icon={<Thermometer size={18} />} color="var(--warning)" value={String(portfolio.zones.length)} label="Zones" />
        <StatCard
      ...
      {facilities.length > 0 && portfolio.loaded && (
        <div className="dashboard-grid">
            ...
            <div className="card-body">
              {totalAlerts === 0 ? (
                <div className="empty-state" style={{ padding: '1.5rem 0' }}>
                  ...
                  <p style={{ color: 'var(--success)', fontWeight: 600 }}>All clear</p>
          ...
          <div className="card">
            <div className="card-header">
              <h3 style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Gauge size={16} /> Demand Overview</h3>
            </div>
            <div className="card-body">
              {portfolio.bills.length === 0 ? (
                <div className="empty-state" style={{ padding: '1.5rem 0' }}>
                  <p className="text-muted">Upload bills to see demand metrics.</p>
                </div>
              ) : (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <div style={{ textAlign: 'center', padding: 16, background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-md)' }}>
                    <Gauge size={24} style={{ color: 'var(--danger)', marginBottom: 4 }} />
                    <div style={{ fontSize: 24, fontWeight: 700 }}>{peakDemand > 0 ? `${peakDemand}` : '—'}</div>
                    <div className="text-muted" style={{ fontSize: 11 }}>Peak kW</div>
                  </div>
                  <div style={{ textAlign: 'center', padding: 16, background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-md)' }}>
                    <TrendingDown size={24} style={{ color: 'var(--warning)', marginBottom: 4 }} />
                    <div style={{ fontSize: 24, fontWeight: 700 }}>${annualDemand > 0 ? annualDemand.toLocaleString() : '—'}</div>
                    <div className="text-muted" style={{ fontSize: 11 }}>Demand Charges</div>
                  </div>
                  <div style={{ textAlign: 'center', padding: 16, background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-md)' }}>
                    <DollarSign size={24} style={{ color: 'var(--accent)', marginBottom: 4 }} />
                    <div style={{ fontSize: 24, fontWeight: 700 }}>
                      {annualCost > 0 && annualDemand > 0
                        ? `${Math.round((annualDemand / annualCost) * 100)}%`
                        : '—'}
                    </div>
                    <div className="text-muted" style={{ fontSize: 11 }}>Demand % of Total</div>
                  </div>
                  <div style={{ textAlign: 'center', padding: 16, background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-md)' }}>
                    <Activity size={24} style={{ color: 'var(--success)', marginBottom: 4 }} />
                    <div style={{ fontSize: 24, fontWeight: 700 }}>{portfolio.bills.length}</div>
                    <div className="text-muted" style={{ fontSize: 11 }}>Bills Analyzed</div>
                  </div>
                </div>
              )}
            </div>
        ...
        {facilities.length === 0 ? (
          ...
          <div className="card">
            ...
            <table className="data-table">
              ...
              <tbody>
                {facilities.map(f => (
                  <tr key={f.id} onClick={() => navigate(`/facilities/${f.id}`)} style={{ cursor: 'pointer' }}>
                    <td>
